...
open class NEBaseChatMessageCell: NEChatBaseCell {
  ...
  public var fullNameLabelRight = UILabel() // 群昵称（只在群聊中有效）
  public var fullNameHRight: NSLayoutConstraint? // 群昵称高度布局约束
  // 已读未读视图
  ...
  open func initProperty() {
    ...
    fullNameLabel.font = UIFont.systemFont(ofSize: 11)
    fullNameLabel.textColor = UIColor.ne_lightText
    fullNameLabel.accessibilityIdentifier = "id.fullNameLabel"

    // fullName
    fullNameLabelRight.translatesAutoresizingMaskIntoConstraints = false
    fullNameLabelRight.font = UIFont.systemFont(ofSize: 11)
    fullNameLabelRight.textColor = UIColor.ne_lightText
    fullNameLabelRight.accessibilityIdentifier = "id.fullNameLabelRight"
...
//      .resizableImage(withCapInsets: UIEdgeInsets(top: 35, left: 25, bottom: 10, right: 25))
  ...
  open func baseCommonUIRight() {
    ...
    contentView.addSubview(fullNameLabelRight)
    fullNameHRight = fullNameLabelRight.heightAnchor.constraint(equalToConstant: 0)
    NSLayoutConstraint.activate([
        fullNameLabelRight.rightAnchor.constraint(equalTo: avatarImageRight.leftAnchor, constant: -chat_content_margin),
        fullNameLabelRight.topAnchor.constraint(equalTo: avatarImageRight.topAnchor),
        fullNameHRight!,
    ])
    ...
    NSLayoutConstraint.activate([
      bubbleImageRight.topAnchor.constraint(equalTo: fullNameLabelRight.bottomAnchor, constant: 4),
...
//    MARK: set data
    ...
    if let avatarURL = model.avatar {
      ...
      if (isSend){
          if model.fullNameHeight > 0 {
            fullNameLabelRight.text = model.fullName
            fullNameLabelRight.isHidden = false
            bubbleTopAnchorLeft?.constant = 0
          } else {
            fullNameLabelRight.text = nil
            fullNameLabelRight.isHidden = true
            bubbleTopAnchorLeft?.constant = 4
          }
          fullNameHRight?.constant = CGFloat(model.fullNameHeight)
      }else{
          if model.fullNameHeight > 0 {
  ...
  open func showLeftOrRight(showRight: Bool) {
    ...
    fullNameLabelRight.isHidden = !showRight
  }
